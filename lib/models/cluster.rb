class Models::Cluster < Models::AbstractModel
  @num_clusters = 10
  ...
  def build_categories(clusters)
    clusters_to_records = Hash[(0..(@num_clusters - 1)).zip(clusters.map do |cluster|
    ...
    categories = clusters_to_records.map do |cluster, records|
      ...
      tf = term_vectors.map { |f| f.inject(&:+) }
    ...
    categories.each_with_index do |category, i|
      if j = categories[0...i].index(category) and categories_records[j]
  ...
  private
    def df(term_vectors)
      term_vectors.map do |f|
        f.inject { |count, tf| tf > 0 ? count + 1 : count }
      end.flatten
    end
    ...
    def bigram_max(records, tf, df)
      ...
      bigrams.max_by do |b|
        b_terms = b.split
        if b == @query or b_terms.include?(@query) or
    ...
    def unigram_max(records, tf, df=false)
      ...
      cluster_terms.max_by do |t|
        if t == @query or @query_terms.include?(t)
          0
        else
          i = labels.index(t)
          if df
    ...
    def gramize(tokens)
      ...
      tokens = tokens.map do |token|
        gram = (last_token and last_token != token) ? "#{last_token} #{token}" : nil
        last_token = token
        gram
      end.compact.uniq
    end
